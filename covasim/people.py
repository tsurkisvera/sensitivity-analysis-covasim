    ...
    **Examples**::
    ...
    def set_prognoses(self):
        ...
        def function(x, a, b, c, d, e):
            return c * beta(a=a, b=b).pdf(d * x + e)
        class Beta:
           def pdf(self, x):
               return function(x, a=2.13275792,  b=4.66586956,  c=0.08,  d=0.40257505, e=-0.07884037)

        dist = Beta()
        urng = np.random.default_rng()
        rng = NumericalInversePolynomial(dist, random_state=urng)
        
        self.rel_sus[:]     =  rng.rvs(self.rel_sus.size)  # Default susceptibilities
