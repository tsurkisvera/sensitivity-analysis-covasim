...
def check_hheads_enough_for_hh(household_sizes, hha_brackets, ages_left_to_assign):
        '''
            CHECK FOR BUG: there're not enough potential heads of households to create all households
        ...
        Returns: 
            nothing, raise the RuntimeError if there're not enough potential heads of households
        '''

        # find the min and max age for household head in hha_brackets
        hha_min = hha_brackets[0][0]
        hha_max = hha_brackets[len(hha_brackets.keys())-1][-1]

        # create new ages_left_to_assign for household heads only (usually without 0-17 years-old people)
        hh_ages_left_to_assign = [ages_left_to_assign[i] for i in range(hha_min, hha_max+1)]
        # sum the number of potential household heads
        max_hh = sum(hh_ages_left_to_assign)

        # print("potential household headers: ", max_hh)
        # print("number of households: ", len(household_sizes))

        #check if there's enough potential household heads
        if max_hh < len(household_sizes):
            raise RuntimeError (f'You have just {max_hh} potential household headers for {len(household_sizes)} households! \n'
                                'Check your household_head_age_distribution_by_family_size and population_age_distributions!')
        return
...
def generate_household_head_ages(household_sizes, hha_by_size, hha_brackets, ages_left_to_assign):
    ...
    household_head_ages = []
    # print(hha_brackets)
    # check household heads number (avoid bug with infinite loop)
    check_hheads_enough_for_hh(household_sizes, hha_brackets, ages_left_to_assign)
    # print(hha_by_size)
    for nh, hs in enumerate(household_sizes):
        hs_distr = hha_by_size[hs - 1, :]
        #print(f'hs_distr: {hs_distr}')
        #print(f'ages_left_to_assign: {ages_left_to_assign}')
        hbi = spsamp.fast_choice(hs_distr)
        #print(f'hbi: {hbi}')
        hbi_distr = np.array([ages_left_to_assign[a] for a in hha_brackets[hbi]])
        #print(f'hbi_distr: {hbi_distr}')
        i = 0
        while sum(hbi_distr) == 0: # pragma: no cover
            ...
            hbi_distr = np.array([ages_left_to_assign[a] for a in hha_brackets[hbi]])
            if i > 100000:
                raise RuntimeError ('There were more then 100000 iterations of while in generate_household_head_ages')

        hha = hha_brackets[hbi][spsamp.fast_choice(hbi_distr)]
        ages_left_to_assign[hha] -= 1

        household_head_ages.append(hha)
        # print(f'hh {nh} is generated', end='\t')
